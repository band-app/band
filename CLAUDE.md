...
- Server shutdown only disconnects. A session ends when its tab is closed, its workspace is deleted (or found deleted at boot), or its shell exits. The daemon exits after 5 idle minutes with no sessions and no connected server, and kills its shells and exits if its endpoint or run dir disappears.

...
// CodeBrowserView explorer column (with its collapse/expand chevron + tab-bar
// toggle) was removed in Phase 2. The sidepanel has no equivalent collapse
// affordance yet. Re-enable when the sidepanel explorer is reworked.
test.describe
  .skip("Files-tab explorer collapsed state", () => {
    ...
    test("collapsing immediately after a drag still persists the dragged width", async ({
      page,
    }) => {

...
test.beforeAll(async () => {
  ...
  seedState(tmpHome, {
    projects: [PROJECT_SPLIT, PROJECT_TITLE, PROJECT_DRAG, PROJECT_CHAT].map(makeRepo),
  });
...
test("panes can be dragged to reorder within the terminal tab", async ({ page }) => {
  ...
  // Two panes with distinct titles so reordering is observable via header text.
  await workspacePage.focusTerminal();
  await workspacePage.splitTerminalRight();
  await expect(workspacePage.terminalPanes()).toHaveCount(2);
  await workspacePage.waitForPanePrompt(0);
  await workspacePage.waitForPanePrompt(1);
  await workspacePage.typeInPane(0, "printf '\\033]0;ALPHA\\007'");
  await workspacePage.typeInPane(1, "printf '\\033]0;BRAVO\\007'");
  await expect
    .poll(() => workspacePage.paneTitles(), { timeout: 20_000 })
    .toEqual(["ALPHA", "BRAVO"]);
  ...
  await expect
    .poll(() => workspacePage.paneTitles(), { timeout: 10_000 })
    .toEqual(["BRAVO", "ALPHA"]);
...
test("⌘D in a chat leaf still splits chat into a sibling group (regression)", async ({ page }) => {
  ...
  // Focus the chat leaf, then ⌘D → a second chat leaf in a sibling group.

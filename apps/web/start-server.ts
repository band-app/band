...
async function main() {
  ...
  if (isDev) {
    ...
    // `src/server/api/` changes, so `/api/openapi.json` reflects
    // router edits during a dev session without a server restart.
    // Mirrors the `server.watcher.on("change")` hook the deleted
    // `trpcDevPlugin` used to install — Vite's HMR handles SSR
    // module invalidation separately, but the spec is generated by
    // `@trpc/openapi`'s static analyser, which has its own cache
    // that only this manual reset can reach.
    //
    // Match by absolute prefix so only edits inside *this* package's
    // API tree invalidate. Pre-fix this used `file.endsWith(
  ...
  // `tunnel.start` procedure in `src/server/api/tunnel/router.ts`
  // reads BAND_PORT to know which port cloudflared should forward

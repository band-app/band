...
async function main() {
  ...
  if (isDev) {
    ...
    // `src/trpc/` OR `src/server/api/` changes, so `/api/openapi.json`
    // reflects router edits during a dev session without a server
    // restart. Mirrors the `server.watcher.on("change")` hook the
    // deleted `trpcDevPlugin` used to install — Vite's HMR handles
    // SSR module invalidation separately, but the spec is generated
    // by `@trpc/openapi`'s static analyser, which has its own cache
    // that only this manual reset can reach.
    //
    // Both trees are watched because Phase 0 of the 3-tier refactor
    // (#311) introduced `src/server/api/router.ts` as the canonical
    // entry point (it's currently a re-export shim for
    // `src/trpc/router.ts`), and Phase 1+ will move real procedures
    // into per-domain sub-routers under `src/server/api/<domain>/`.
    // Watching only `src/trpc/` here would silently let those Phase
    // 1+ edits serve stale OpenAPI in dev sessions.
    //
    // Match by absolute prefix so only edits inside *this* package's
    // tRPC trees invalidate. Pre-fix this used `file.endsWith(
    ...
    const apiDir = join(SERVER_ROOT, "src", "server", "api");
    viteServer.watcher.on("change", (file: string) => {
      if (file.startsWith(trpcDir) || file.startsWith(apiDir)) {
        _openApiSpec = null;
      }
    });
  } else {
  ...
  // Stash the actual bound port for downstream consumers (the
  // `tunnel.start` procedure — implemented in `src/trpc/router.ts`
  // today, exposed via the canonical entry point at
  // `src/server/api/router.ts` — reads BAND_PORT to know which port
  // cloudflared should forward to). Set AFTER `listenWithFallback`

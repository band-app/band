...
pub fn compute_layout(app_sizes: &[f64], screen_width: i32, screen_height: i32) -> Vec<ScreenRect> {
    ...
    for (i, &size) in app_sizes.iter().enumerate() {
        let width = if i == app_sizes.len() - 1 {
            // Last app absorbs rounding remainder
            screen_width - x
        } else {
...
pub fn raise_window_by_bundle_id(bundle_id: &str, folder_name: &str) {
...
pub fn position_window_by_bundle_id(
    bundle_id: &str,
    folder_name: &str,
    rect: &ScreenRect,
) -> Result<(), String> {
...
/// Find and focus a window using System Events (`AXRaise` first, then activate).
/// This avoids the bug where `tell application ... activate` opens a new window
/// or steals focus before we know if a matching window exists.
pub fn find_and_focus_window(bundle_id: &str, folder_name: &str) -> Result<bool, String> {
    let script = format!(
        r#"tell application "System Events"
    if not (exists (first process whose bundle identifier is "{bundle_id}")) then
        return "no_app"
    end if
    tell (first process whose bundle identifier is "{bundle_id}")

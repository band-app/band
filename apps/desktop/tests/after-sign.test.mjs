 ...
 *
 * The afterSign hook is electron-builder's primary signing-completion seam.
 ...
 *     binary (delegated to deep-sign-mac.mjs), then submits the .app to
 *     notarytool (delegated to notarize.mjs).
...
describe("afterSign hook", () => {
  ...
  test("darwin: tolerates an .app without Resources/web/ (no extraResources)", async () => {
    if (process.platform !== "darwin") return;
    const dir = await mkdtemp(join(tmpdir(), "band-after-sign-noweb-"));
    try {
  ...
  test("darwin signed build: signs web/ tree AND the CLI sidecar at binaries/band", async () => {
    if (process.platform !== "darwin") return;
    const dir = await mkdtemp(join(tmpdir(), "band-after-sign-orch-"));
    try {
      const { web, cli } = await makeAppWithWeb(dir);
      // electron-builder also writes its own entitlements file under the
      // standard build/ path — the after-sign hook walks up to find it,
  ...
  test("darwin signed build with no CLI sidecar: signs only the web/ tree", async () => {
    if (process.platform !== "darwin") return;
    const dir = await mkdtemp(join(tmpdir(), "band-after-sign-nocli-"));
    try {

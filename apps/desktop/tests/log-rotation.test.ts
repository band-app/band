 ...
 *
 * Real filesystem, real `~/.band/desktop.log` (sandboxed via HOME). Asserts
 * that a >5MB log gets rotated to `.old` on next write.
 */
...
import { desktopLogPath, logToFile } from "../src/main/services/log.ts";
...
describe("log rotation", () => {
  ...
  test("rotates to .old when log exceeds 5MB", async () => {
    ...
    // Triggering one more write should observe the rotation.
    logToFile("post-rotation entry");
